    ...
    public class ResourceController : BaseController<ResourceController, Data.SteponContext, ApplicationUser, IdentityRole>
    {
        ...
        public JsonResult GetBackupResource()
        {
            try
            {
                ...
                {
                    return Json(new { result = false, message = "暂无数据" });
                }
            }
            catch (Exception e)
            {
                ...
                if (Directory.Exists(filepath))
                {
                    ...
                    foreach (FileInfo NextFolder in Files)
                    {
                        FileList iil = new FileList
                        {
                            ...
                            FileUrl = weburl + "/upload/Resource/" + data.Id + "/" + NextFolder.Name
                        };
                        filelist.Add(iil);
                    }
        ...
        public string DownloadModelFile()
        {
            ...
            {
                var weburl = _configuration.GetSection("Message:WebUrl").Value;
                var filepath = System.IO.Path.Combine(weburl + "/upload/Resource", "资源模板.xlsx");
        ...
        public string ExportResource(string id)
        {
            try
            {
                ...
                var weburl = _configuration.GetSection("Message:WebUrl").Value;
                var urlpath = weburl + "/upload/Resource/导出文件/" + loginusername + ".zip";
                return urlpath;
            }
            catch (Exception e)
            {
