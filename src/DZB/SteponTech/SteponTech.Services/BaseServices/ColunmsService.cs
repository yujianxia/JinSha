...
namespace SteponTech.Services.BaseServices
{
    public class ColunmsService : ServiceContract<SteponContext>
    {
        ...
        {
            return _context.ColunmsView.ToList();
        }
        /// <summary>
        /// 获取所有栏目
        /// </summary>
        ...
        {
            return _context.Colunms.Where(e => e.Id != null).ToList();
        }
        /// <summary>
        /// 根据ID获取栏目信息
        /// </summary>
        ...
        {
            return _context.Colunms.FirstOrDefault(e => e.Id == id);
        }
        /// <summary>
        /// 增加栏目
        /// </summary>
        ...
        {
            var result = new OperationResult();
            var nextcolunms = _context.ColunmsView.Where(x => x.ColunmsId == id).FirstOrDefault();
            if (nextcolunms == null)
            {
                if (_context.Information.Where(x => x.ColumnId == id).FirstOrDefault() != null)
                {
                    result.State = OperationState.Faild;
        ...
        {
            return _context.Colunms.Where(e => e.ModelId == modelId).OrderBy(e => e.Sorting).ToList();
        }
    }
}
