...
namespace SteponTech.ApiControllers.Admin
{
    ...
    public class ColunmsController : BaseController<ColunmsController, Data.SteponContext, ApplicationUser, IdentityRole>
    {
        ...
        /// <returns></returns>
        private void RecursiveFind(System.Collections.Queue childrenQueue, System.Collections.Generic.List<ColunmsYoungView> allPermissions, Guid parentMark, int level)
        {
        ...
        /// <returns></returns>
        [HttpPost("[action]")]
        public JsonResult AddColunms([FromBody]ColunmsYoung Colunms)
        {
            var isresult = false;
            var message = "添加失败";
            try
        ...
        /// <returns></returns>
        [HttpPost("[action]")]
        public JsonResult UpdateColunms([FromBody]ColunmsYoung Colunms)
        {
            var isresult = false;
            var message = "修改失败！";
            try
        ...
        public JsonResult UpdateSort(Guid id, int sort)
        {
            ...
            try
            {
                ...
                {
                    colunm.Sorting = sort;
                    var result = brandService.ModifyColunms(colunm);
                    isresult = result.State == OperationState.Success;
                    message = result.Message;
