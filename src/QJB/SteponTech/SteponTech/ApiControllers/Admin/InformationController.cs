...
namespace SteponTech.ApiControllers.Admin
{
    ...
    public class InformationController : BaseController<InformationController, Data.SteponContext, ApplicationUser, IdentityRole>
    {
        ...
        public JsonResult GetAllInformation()
        {
            try
            {
                ...
                if (data?.Count > 0)
                {
                    ...
                    var newdata = data.Select(e => new { e.Title, e.Music }); 
        ...
        /// <returns></returns>
        [HttpPost("[action]")]
        public JsonResult AddInformation([FromBody]InformationYoung Information)
        {
            var isresult = false;
            var message = "添加失败";
            try
        ...
        /// <returns></returns>
        [HttpPost("[action]")]
        public JsonResult UpdateInformation([FromBody]InformationYoung Information)
        {
            var isresult = false;
            var message = "修改失败！";
            try
        ...
        public JsonResult GetInformation(Guid id)
        {
            try
            {
                InformationYoungViewService brandService = new InformationYoungViewService(Services);
                ...
                if (Directory.Exists(filepath))
                {
                    ...
                    foreach (FileInfo NextFolder in Files)
                    {
                        FileList iil = new FileList
                        {
                            ...
                            FileUrl = weburl + "/upload/Information/" + data.Id + "/" + NextFolder.Name
                        };
                        filelist.Add(iil);
                    }
        ...
        /// <returns></returns>
        [HttpGet("[action]/{id}")]
        public JsonResult GetSunInformation(Guid id)
        {
            try
            {
                //var name = User.Identity.Name;
                InformationService userService = new InformationService(Services);
                var data = userService.GetInformationByColumnId(id);
                if (data?.Count > 0)
                {
                    //var newdata = data.Select(e => new { e.Id, e.Name });
